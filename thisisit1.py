...
regressor=RandomForestClassifier(n_estimators=10000,criterion = 'entropy',random_state=0)
...
regressor=ExtraTreesClassifier(n_estimators=10000,max_features= 100,criterion= 'entropy',min_samples_split= 2,max_depth= 50, min_samples_leaf= 5)
regressor.fit(X_train,Y_train)
...
Prediction = {'s.no': [str(i) for i in range(1,260)],
        'treatment': ["Yes" if j==1 else "No" for j in Y_final_pred]
        }

df = DataFrame(Prediction, columns= ['s.no', 'treatment'])
export_csv = df.to_csv ('./result6.csv', index = None, header=True)

!ls

from google.colab import files
files.download('result6.csv')